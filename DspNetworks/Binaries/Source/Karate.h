...
template <int NV>
using Focus = parameter::plain<Karate_impl::focus_smooth_t<NV>, 
                               0>;
template <int NV>
...
template <int NV> struct instance: public Karate_impl::Karate_t_<NV>
{
	...
	struct metadata
	{
		...
		SNEX_METADATA_ENCODED_PARAMETERS(78)
		{
            ...
            0x8000, 0xE33F, 0xE0F5, 0x003E, 0x8000, 0x003F, 0x0000, 0x5B00, 
            0x0001, 0x0000, 0x694D, 0x0078, 0x0000, 0x0000, 0x0000, 0x3F80, 
            0x0000, 0x3F80, 0x0000, 0x3F80, 0x0000, 0x0000, 0x025B, 0x0000, 
            0x4100, 0x7269, 0x0000, 0x0000, 0x0000, 0x8000, 0xC13F, 0x5EA2, 
            0x003E, 0x8000, 0x003F, 0x0000, 0x5B00, 0x0003, 0x0000, 0x7250, 
            0x7365, 0x7573, 0x6572, 0x0000, 0x0000, 0x0000, 0x8000, 0xA93F, 
            0x0F25, 0x003F, 0x8000, 0x003F, 0x0000, 0x5B00, 0x0004, 0x0000, 
            0x7053, 0x6565, 0x0064, 0x0000, 0x0000, 0x0000, 0x3F80, 0x4B44, 
            0x3B58, 0x0000, 0x3F80, 0x0000, 0x0000, 0x0000
		};
	};
	...
	instance()
	{
		...
		auto& air = this->getT(0).getT(1).getT(1);                                    // Karate_impl::air_t<NV>
		auto& minmax = this->getT(0).getT(1).getT(2);                                 // Karate_impl::minmax_t<NV>
		...
		auto& freq_split3 = this->getT(3).getT(1).getT(5).getT(0).getT(1).getT(0);    // Karate_impl::freq_split3_t<NV>
		...
		auto& band2 = this->getT(3).getT(1).getT(5).getT(0).getT(1).getT(0).getT(1); // Karate_impl::band2_t<NV>
		...
		auto& gain2 = this->getT(3).getT(1).getT(5).getT(0).                     // core::gain<NV>
		...
		auto& wet_gain = this->getT(3).getT(1).getT(7);                          // core::gain<NV>
		...
		this->getParameterT(0).connectT(0, focus_smooth);      // Focus -> focus_smooth::Value
		...
		pressure_smooth.getParameter().connectT(0, minmax1);         // pressure_smooth -> minmax1::Value
		pressure_smooth.getParameter().connectT(1, DrumSlam);        // pressure_smooth -> DrumSlam::Drive
		focus_smooth.getParameter().connectT(0, minmax2);            // focus_smooth -> minmax2::Value
		focus_smooth.getParameter().connectT(1, minmax);             // focus_smooth -> minmax::Value
		focus_smooth.getParameter().connectT(2, Focus);              // focus_smooth -> Focus::Boost
		...
		minmax1.setParameterT(1, -6.6);     // control::minmax::Minimum
		minmax1.setParameterT(2, 2.);       // control::minmax::Maximum
		minmax1.setParameterT(3, 0.362858); // control::minmax::Skew
		...
		minmax.setParameterT(1, -9.4);    // control::minmax::Minimum
		minmax.setParameterT(2, 0.);      // control::minmax::Maximum
		minmax.setParameterT(3, 1.81393); // control::minmax::Skew
		...
		focus_smooth.setParameterT(1, 100.); // control::smoothed_parameter::SmoothingTime
		...
		Focus.setParameterT(1, 0.443645); // project::Focus::Focus
		...
		this->setParameterT(0, 0.439376);
		this->setParameterT(1, 1.);
		this->setParameterT(2, 0.217418);
		this->setParameterT(3, 0.559168);
