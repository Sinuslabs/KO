namespace ZoomHandler
{
	...
	ZoomPanel.setPaintRoutine(function(g) {
		var DRAGGER_COLOUR = Theme.THEME.Colors.UI.on_background_var;
	...
	ZoomPanel.setMouseCallback(function(event) {
		...
		if(event.drag) {
			...
			if (Math.round(currentZoom * 100) / 100 != zoomToUse) {
				Console.print(Math.round(currentZoom * 100) / 100 + ' : ' + zoomToUse);
				Settings.setZoomLevel(Math.round(zoomToUse * 100) / 100);
				UserSettings.save('zoom', Math.round(zoomToUse * 100) / 100);
			}
		}
